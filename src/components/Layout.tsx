...
export default function Layout({ children }: { children: ReactNode }) {
  ...
  const navCls = ({ isActive }: { isActive: boolean }) =>
    `flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium transition-colors ${
      isActive ? 'bg-indigo-950/60 text-indigo-300' : 'text-slate-300 hover:bg-slate-800'
    }`

  return (
    <div className="flex min-h-screen flex-col">
      <header className="relative sticky top-0 z-20 flex items-center gap-3 border-b border-slate-700 bg-slate-900 px-4 py-2.5">
        <span className="text-lg font-bold whitespace-nowrap text-indigo-400">📋 {t('appName')}</span>
        ...
        <div className="mx-2 flex min-w-0 flex-1 justify-end">
          <SearchBox />
        </div>
      ...
      {/* mobile bottom nav */}
      <nav className="fixed inset-x-0 bottom-0 z-20 flex border-t border-slate-700 bg-slate-900 sm:hidden">
        {[
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex flex-1 flex-col items-center gap-0.5 py-2 text-[10px] font-medium ${isActive ? 'text-indigo-400' : 'text-slate-400'}`
            }
          >
